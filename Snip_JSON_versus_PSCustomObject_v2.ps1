...
Function Main 
   ...
   # if ( $JsonString -eq ""  -or $JsonString -eq $Null ) { Return -1 }
    ...
    [String]$SingleOrArray = JsonString___Array_of_JSONObjects__or__Single_JSONObject -JsonString $JsonString
    # Write-Host  $JsonString
    $AllJsonStrings = @()
    if ( $SingleOrArray -eq 'Single' )     { $AllJsonStrings += $JsonString   }
    elseif ( $SingleOrArray -eq 'Array' )  { 
        $AllJsonObjects =  $JsonString | ConvertFrom-JSON
        foreach( $JsonObject in $AllJsonObjects )
            {
            $JsonString = $JsonObject | ConvertTo-JSON
            $AllJsonStrings += $JsonString 
            }
        }

if ( 1 -eq 1 ) # JsonString - add a NoteItems (NodeProperty) from JsonString
    {
    ForEach ( $JsonString in $AllJsonStrings )
        {
        # Remove current object
        $AllJsonStrings = $AllJsonStrings -ne $JsonString
        Write-Host "--------------------"
...
if ( 1 -eq 1 ) # JsonString - add a new NodeProperty "NewProperty" with value "Value-NewProperty"
    {    
    # #####################################################
...
if ( 1 -eq 2) # add a new SubNode 'SubNodeName' with different and properties and values
    {
    # #####################################################
...
if ( 1 -eq 2 ) # query Sub-Sub-...Sub-Nodes and values
    {
...
Function JsonString___Array_of_JSONObjects__or__Single_JSONObject 
    ...
    <#
        # 'Single' Object or 'Array' of objects
        [String]$SingleOrArray = JsonString___Array_of_JSONObjects__or__Single_JSONObject -JsonString $FirmwareExample1 
        # Write-Host  $JsonString
        if ( $SingleOrArray -eq 'Single' )     {  $S1 =" bla bla bla ..."   }
    #>

    Write-Host  $JsonString
    
    $JsonPsObj = ($JsonString | ConvertFrom-JSON)
    if ( $JsonPsObj -is [system.array] )
        {
        Write-Host "Multiple objects"
        foreach( $Obj in $JsonPsObj )
            {
            Write-Host $Obj 
